...
function* getDiscoverSaga(action: ReturnType<typeof movieActions.getDiscoverSaga>) {
  try {
    yield put(movieActions.setLoading(true));
...
function* findMoviesSaga(action: ReturnType<typeof movieActions.findMoviesSaga>) {
  try {
    yield put(movieActions.setLoading(true));
